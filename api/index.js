...
module.exports = async (req, res) => {
  ...
  try {
    const searchUrl = `https://www.gsmarena.com/results.php3?sQuickSearch=yes&sName=${encodeURIComponent(model)}`;
    const { data: searchData } = await axios.get(searchUrl);
    const $ = cheerio.load(searchData);
    
    // Naya selector jo search results ki pehli link uthayega
    const firstResult = $('.makers .makers-list li a').first().attr('href');
    
    if (!firstResult) {
      return res.status(404).json({ status: "error", message: "Phone nahi mila, shayad naam galat hai." });
    }
